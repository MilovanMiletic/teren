...
describe('CompaniesPage', () => {
  ...
  describe('the info popover', () => {
    it('explains what a suspension does, on a tap', async () => {
      ...
      expect(element.querySelector('.pop')?.textContent).toContain('Telefoni se sami odjavljuju');
    });
  });
  ...
  it('ignores a failed read that lands after a good one', async () => {
    ...
    vi.spyOn(gateway, 'listCompanies').mockImplementation(async () => {
      call += 1;
      if (call === 1) {
        await slow;
        throw platformHttpError(503);
      }
      return gateway.real.listCompanies({});
    });
