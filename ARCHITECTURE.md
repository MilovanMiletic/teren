...
| **Dev (`dev.teren.rs`)** | Small VPS running the same compose stack | B3a — **machinery built and proven locally 2026-08-30; decisions made 2026-09-03 (`teren.rs` registered now, Resend as the relay); no host bought yet** | Runs continuously without the laptop; where background jobs, email and the demo actually get exercised, and where everything is tested before the first client |
| **Production** | Same stack, the `teren.rs` apex, backups and alerting | C7 | Real customers. The apex is pointed here only when the app is production-ready; the domain itself is registered at B3a, because `dev.teren.rs` is a subdomain of it and Resend must verify the sending domain |
